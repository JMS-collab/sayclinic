...
export default function Calendar({ 
  ...
  // POMOCNÉ VÝPOČTY PRE RESIZE A ČASOVÚ OS
  ...
  // --- POHĽAD: DEŇ (PREHĽADNÁ ČASOVÁ OS S RESIZE MOŽNOSŤOU) ---
  const renderDayView = () => {
            ...
            {dayEvents.map(evt => {
              ...
              return (
                <div 
                  ...
                  className={`absolute left-3 right-3 rounded-xl p-3 shadow-md border pointer-events-auto cursor-pointer transition-all hover:scale-[1.002] flex flex-col justify-between group ${
                    evt.isCancelled 
                      ...
                      : evt.type === 'osetrenie'
                      ? 'bg-emerald-50/60 border-emerald-500 border-l-8 border-l-emerald-600'
                      : 'bg-blue-50/60 border-blue-400 border-l-8 border-l-blue-500'
                  }`}
                >
                  <div className="flex justify-between items-center gap-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-[#2C2A29] bg-gray-100 px-2 py-0.5 rounded">
                        {evt.startTime} - {evt.endTime}
                      </span>
                      {getEventTypeBadge(evt.type)}
                      {evt.isCancelled && <span className="text-rose-700 text-[9px] font-bold">❌ ZRUŠENÉ</span>}
                    </div>

                    <div className="flex items-center gap-2 font-mono text-xs font-bold">
                      <span>{price} €</span>
                      {deposit > 0 && (
                        <span className={`text-[9px] px-1.5 py-0.5 rounded border ${isPaid ? 'bg-emerald-100 border-emerald-300 text-emerald-800' : 'bg-rose-100 border-rose-300 text-rose-800'}`}>
                          {isPaid ? 'Záloha ✓' : 'Záloha ✕'}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="my-1">
                    <h4 className="font-bold text-sm text-[#2C2A29] truncate">{evt.title}</h4>
                    <p className="text-xs text-[#8C857B] truncate">{evt.patientName} | {evt.doctorName}</p>
                  </div>
    ...
    <div className="bg-white p-6 rounded-2xl border border-[#E8E2D9] shadow-sm space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#E8E2D9] pb-4">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-widest text-[#8C857B]">Vizuálna časová os, naťahovanie času, zrušenie termínov a podklady</p>
        </div>
        ...
        <div className="fixed inset-0 bg-[#2C2A29]/60 flex items-center justify-center z-50 backdrop-blur-sm p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-lg shadow-xl border border-[#E8E2D9] space-y-4">
            ...
            <div className="space-y-2 text-xs bg-[#FBF9F6] p-4 rounded-xl border border-[#E8E2D9]">
              <p><strong>Typ:</strong> {getEventTypeLabel(selectedEvent.type)}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
              <button onClick={() => sendWhatsApp(selectedEvent)} className="bg-emerald-600 text-white py-2 rounded-xl text-xs font-bold uppercase">💬 WA</button>
              <button onClick={() => sendSMS(selectedEvent)} className="bg-[#2C2A29] text-white py-2 rounded-xl text-xs font-bold uppercase">📲 SMS</button>
              <button onClick={() => { setOpenEmailEventId(selectedEvent.id); setSelectedEvent(null); }} className="bg-[#C5A059] text-white py-2 rounded-xl text-xs font-bold uppercase">✉️ E-mail</button>
              <button onClick={() => { setCancellingEvent(selectedEvent); setSelectedEvent(null); }} className="bg-rose-50 text-rose-700 border border-rose-200 py-2 rounded-xl text-xs font-bold uppercase">❌ Zrušiť</button>
            </div>

            <div className="flex justify-between items-center pt-2 border-t border-[#E8E2D9]">
              <button onClick={() => { setEditingEventData(selectedEvent); setIsEditingEvent(true); setSelectedEvent(null); }} className="text-xs text-[#C5A059] font-bold uppercase underline">
                ✏️ Upraviť údaje / čas
              </button>
              <button onClick={() => { handleOpenFolderForEvent(selectedEvent); setSelectedEvent(null); }} className="bg-[#2C2A29] text-white px-4 py-2 rounded-xl text-xs font-bold uppercase">
                📁 Karta pacienta
              </button>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-xl border border-[#E8E2D9] space-y-4">
            ...
            <form onSubmit={handleConfirmCancelEvent} className="space-y-3 text-xs">
              ...
              {cancelReasonInput === 'Iné (vlastný dôvod)' && (
                <div>
                  <input type="text" required placeholder="Dôvod..." value={customCancelReason} onChange={e => setCustomCancelReason(e.target.value)} className="w-full border p-2 rounded-lg bg-[#FBF9F6]" />
                </div>
              )}
