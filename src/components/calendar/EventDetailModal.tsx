...
interface EventDetailModalProps {
  ...
  onToggleDepositPaid: (eventId: string, newStatus: boolean) => void;
        ...
        <div className="border-b border-[#E8E2D9] pb-3 flex justify-between items-start shrink-0">
          <div>
            <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
              ...
              {getEventTypeBadge(event.type, event.freeformCategory)}
          ...
          <button 
            type="button"
            onClick={onClose} 
            className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 flex items-center justify-center font-bold text-sm transition-colors shrink-0 ml-2"
          >
            ✕
          </button>
        </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-indigo-200/60">
                <div>
                  ...
                  <span className="font-semibold text-indigo-950">
                    {event.date} • {event.isAllDay ? 'Celý deň' : `${event.startTime} - ${event.endTime} (${durationMin} min)`}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-bold text-indigo-700 block">Komu je pridelená</span>
                  <span className="font-semibold text-indigo-950">
              ...
              <div className="bg-[#FBF9F6] p-3.5 rounded-xl border border-[#E8E2D9] space-y-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <div>
                    ...
                    {getEventPhone(event) && (
                      <span className="text-[11px] text-[#8C857B] font-mono block">📞 {getEventPhone(event)}</span>
                    )}
                    {getEventEmail(event) && (
                      <span className="text-[11px] text-[#8C857B] block">✉️ {getEventEmail(event)}</span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <span className="font-bold text-[#2C2A29]">
                      {new Date(event.date).toLocaleDateString('sk-SK', { weekday: 'short', day: 'numeric', month: 'numeric', year: 'numeric' })}
                    </span>
            ...
            <div className="space-y-2 pt-1">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button 
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-xl text-[10px] font-bold uppercase shadow-2xs transition-colors flex items-center justify-center gap-1"
                >
                  <span>💬</span> WhatsApp
                </button>
                <button 
                  ...
                  className="bg-[#2C2A29] hover:bg-[#C5A059] text-white py-2 rounded-xl text-[10px] font-bold uppercase shadow-2xs transition-colors flex items-center justify-center gap-1"
                >
                  <span>📲</span> SMS správa
                </button>
                <button 
                  type="button" 
                  onClick={() => onToggleEmailPanel(event)} 
                  className="bg-[#C5A059] hover:bg-[#b08d4b] text-white py-2 rounded-xl text-[10px] font-bold uppercase shadow-2xs transition-colors flex items-center justify-center gap-1"
                >
                  <span>✉️</span> E-mail
                </button>
              </div>
              ...
              {event.depositAmount && event.depositAmount > 0 && (
                <button
                  type="button"
                  onClick={() => onToggleDepositPaid(event.id, !event.isDepositPaid)}
                  className={`w-full py-2 rounded-xl text-[10px] font-bold uppercase border transition-colors mt-1 ${
                    event.isDepositPaid 
            ...
            <div className="bg-[#FBF9F6] border border-[#C5A059] p-3.5 rounded-xl space-y-2.5 text-xs animate-fadeIn">
              <div className="flex justify-between items-center border-b border-[#E8E2D9] pb-1.5">
                ...
                <button 
                  type="button" 
                  onClick={() => onToggleEmailPanel(event)} 
                  className="text-xs font-bold text-[#8C857B]"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="flex justify-end">
                <button 
                  ...
                  onClick={() => onSendEmailSubmit(event)} 
                  className="bg-[#C5A059] hover:bg-[#b08d4b] text-white px-4 py-1.5 rounded-lg font-bold text-[10px] uppercase shadow-2xs transition-colors disabled:opacity-50"
                >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-[#E8E2D9] shrink-0">
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-white hover:bg-gray-100 text-[#2C2A29] border border-[#E8E2D9] px-3.5 py-2 rounded-xl text-[10px] font-bold uppercase shadow-2xs transition-colors flex items-center gap-1"
            >
              <span>✏️</span> Upraviť
            </button>
            ...
            {!event.isCancelled && (
              <button
                ...
                className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 px-3 py-2 rounded-xl text-[10px] font-bold uppercase transition-colors"
              >
                ✕ Zrušiť termín
              </button>
            )}
          ...
          <div className="flex items-center gap-2">
            {event.type !== 'volno' && (
              <button
                type="button"
                onClick={() => onOpenFolder(event)}
                className="bg-[#2C2A29] hover:bg-[#C5A059] text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase shadow-2xs transition-colors flex items-center gap-1"
              >
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-2 rounded-xl text-[10px] font-bold uppercase text-[#8C857B] hover:text-[#2C2A29]"
            >
              Zavrieť
            </button>
          </div>
